...
# 本文件目录: .../recommender_project/experiments/bookcrossing
...
from preprocess_bookcrossing import build_datasets_and_loaders_bookcrossing
...
def main():
    # 注意这里用 PROJECT_ROOT（项目根目录），不是 EXP_DIR
    ...
    print("Loading Book-Crossing data (user features = AGE ONLY)...")
    ...
    # meta["user_features"] 当前是 [n_users, 22]，第 0 列是 age_norm
    user_features_full = meta["user_features"]
    user_features_np = user_features_full[:, :1]  # 只取 age 这一列
    print(
        f"Book-Crossing: n_users={n_users}, n_items={n_items}, "
        f"user_feat_dim={user_features_np.shape[1]} (AGE ONLY)"
    )

    # 转成 tensor，放到 device
    user_features = torch.from_numpy(user_features_np).float().to(device)

    print("Building NCFUserFeat model (AGE ONLY)...")
    model = NCFUserFeat(
        num_users=n_users,
        num_items=n_items,
        user_feat_dim=user_features.shape[1],  # =1
        emb_dim=cfg["model"].get("emb_dim", 32),
        mlp_layer_sizes=tuple(cfg["model"]["mlp_layers"]),
        dropout=cfg["model"].get("dropout", 0.0),
    ).to(device)
    model.set_user_features(user_features)
    ...
    # 模型仍然保存到根目录，方便后面 evaluate 脚本统一读取
    save_path = os.path.join(PROJECT_ROOT, "ncf_user_age_only_bookcrossing_best.pth")
    torch.save(
        {
    ...
    print(f"Saved AGE-ONLY NCFUserFeat model to {save_path}")
