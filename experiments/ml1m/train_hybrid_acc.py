# experiments/ml1m/train_hybrid_acc.py
# -*- coding: utf-8 -*-
"""
训练 HybridNCFAcc（准确性优先）。

相对 train_hybrid.py 的改动点：
1) 支持 AMP + 少一次 forward（已在你的 train_hybrid.py 版本里实现）；
2) 增加对齐损失 L_align（InfoNCE），只对高评分样本/子采样计算，避免 batch 过大带来 O(B^2) 的成本；
3) 读取 config.yaml 的 hybrid_acc 节；保存到 paths.hybrid_acc_ckpt / paths.hybrid_acc_ckpt_safe。

说明：
- 不改变 HybridNCF 的核心打分/融合逻辑，只是训练目标更“把内容拉进来”。
"""
...
try:
    from models.hybrid_acc_model import HybridNCFAcc
except Exception:
...
def train_epoch(
    ...
    for u, i, r in loader:
        ...
        with ctx:
            # 1) rating loss：只算一次 score_logits
            ...
            # 2) BPR loss：只对高评分样本，pos_logit 复用 logit_all
            ...
            # 3) Align loss：只对高评分样本的 item 计算；并做子采样避免 O(B^2) 过慢
            if lambda_align > 0 and model.has_align():
                pos_mask = (r >= float(pos_rating_threshold))
                if pos_mask.any():
                    items_pos = i[pos_mask]
                    if align_max_batch and align_max_batch > 0 and items_pos.numel() > int(align_max_batch):
                        # 随机子采样
                        perm = torch.randperm(items_pos.numel(), device=items_pos.device)[: int(align_max_batch)]
                        items_pos = items_pos[perm]
                    loss_align = model.align_loss(items_pos, temperature=float(align_temperature))
                else:
                    loss_align = torch.tensor(0.0, device=device)
            else:
                loss_align = torch.tensor(0.0, device=device)

            loss = float(lambda_rating) * loss_rating + float(lambda_bpr) * loss_bpr + float(lambda_align) * loss_align
...
def main():
    ...
    hy = cfg.get("hybrid_acc", cfg.get("hybrid", {}))
    ...
    lambda_bpr = float(hy.get("lambda_bpr", 0.8))
    pos_rating_threshold = float(hy.get("pos_rating_threshold", 4.0))

    # Acc 关键：align
    lambda_align = float(hy.get("lambda_align", 0.05))
    align_temperature = float(hy.get("align_temperature", 0.2))
    align_max_batch = int(hy.get("align_max_batch", 256))
    ...
    for epoch in range(1, epochs + 1):
        ...
        tr_loss = train_epoch(
            ...
            align_max_batch=align_max_batch,
            pos_rating_threshold=pos_rating_threshold,
            grad_clip=grad_clip,
            rating_min=rating_min,
            rating_max=rating_max,
        ...
        dt = time.time() - t0
        print(f"[Acc][Epoch {epoch:03d}] loss={tr_loss:.4f} val_RMSE={val_rmse:.4f} val_MAE={val_mae:.4f} ({dt:.1f}s)")
    ...
    print(f"[Acc][Test] RMSE={test_rmse:.4f}, MAE={test_mae:.4f}")

    ckpt_name = cfg.get("paths", {}).get("hybrid_acc_ckpt", "hybrid_acc_ml1m_best.pth")
    ckpt_safe_name = cfg.get("paths", {}).get("hybrid_acc_ckpt_safe", "hybrid_acc_ml1m_best_safe.pth")
