# experiments/ml1m/train_hybrid_tail.py
# -*- coding: utf-8 -*-
"""
训练 HybridNCFTail（长尾/新颖性/覆盖优先）。

相对 train_hybrid.py 的改动点：
1) 使用 HybridNCFTail（在 logit 上加入流行度惩罚项）；
2) item_popularity 来自 train 集（不看 val/test），写入 ckpt，评估端可覆盖为 train+val 统计；
3) 读取 config.yaml 的 hybrid_tail 节；保存到 paths.hybrid_tail_ckpt / paths.hybrid_tail_ckpt_safe。
"""
...
try:
    from models.hybrid_tail_model import HybridNCFTail
except Exception:
    ...
    for u, i, r in loader:
        ...
        with ctx:
            ...
            if lambda_bpr > 0 and num_neg > 0:
                ...
                if pos_mask.any():
                    u_pos = u[pos_mask]
                    pos_logit = logit_all[pos_mask].unsqueeze(1)

                    neg_items = sample_negatives(u_pos, user_pos_sets, n_items, num_neg, device)
...
def compute_item_popularity_train(train_df, item2idx, n_items: int) -> np.ndarray:
...
def main():
    ...
    # train 集 item popularity（不看 val/test）
    item_pop_train = compute_item_popularity_train(train_df, item2idx=item2idx, n_items=n_items)
    ...
    hy = cfg.get("hybrid_tail", cfg.get("hybrid", {}))
    ...
    print(
        f"[Tail cfg] pop_alpha={pop_alpha} (learnable={learnable_pop_alpha}), "
        f"pop_mode={pop_mode}, user_pop_scaling={user_pop_scaling}, "
        f"user_pop_scale_range={tuple(float(x) for x in user_pop_scale_range)}"
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        dt = time.time() - t0
        print(f"[Tail][Epoch {epoch:03d}] loss={tr_loss:.4f} val_RMSE={val_rmse:.4f} val_MAE={val_mae:.4f} ({dt:.1f}s)")
    ...
    print(f"[Tail][Test] RMSE={test_rmse:.4f}, MAE={test_mae:.4f}")

    ckpt_name = cfg.get("paths", {}).get("hybrid_tail_ckpt", "hybrid_tail_ml1m_best.pth")
    ckpt_safe_name = cfg.get("paths", {}).get("hybrid_tail_ckpt_safe", "hybrid_tail_ml1m_best_safe.pth")
